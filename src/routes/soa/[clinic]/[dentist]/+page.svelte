<script lang="ts">
	...
	let keys = ['patientName', 'patientContact', 'description', 'caseNotes', 'caseStatus', 'datePickup'];
    let displayKeys = ['Patient Name', 'Patient Contact', 'Description', 'Case Notes', 'Case Status', 'Date Pickup'];
</script>
...
<div id="printarea" class="flex flex-col">
	<div class="grid-row-3 grid px-2">
		...
		<div class="text-center">
            ...
            <h2 class="text-2xl mt-2 text-slate-600">{data.dentistName}</h2>
		</div>
	</div>
			...
			<tbody>
				{#each table as row, index}
					<tr class="border-b border-border bg-white dark:border-secondary-dark dark:bg-primary-dark">
						{#each keys as key}
							<td class="max-w-[150px] truncate px-6 py-4">{(row as any)[key] || ((row as any).records)?.[key] || ''}</td>
						{/each}
						<td class="px-6 py-4 print:hidden">
							<a aria-label="history" href={`/history/${(row as any).recordId || ((row as any).records)?.recordId}`}>
								LINK
							</a></td
						>
					</tr>
	...
	{:else}
		<div class="mx-5 mt-10 flex flex-col items-center justify-center rounded-xl border border-dashed border-border bg-surface/50 py-12 text-center">
			...
			<p class="mt-1 text-sm text-text-muted">There are no statements of account available for this clinic and dentist.</p>
		</div>
	{/if}
</div>
