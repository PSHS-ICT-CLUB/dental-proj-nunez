...
<div class="mx-auto max-w-6xl px-4 py-8">
	<div class="mb-6 flex items-center justify-between">
		...
		<div class="flex gap-2">
			<a href="/inventory" class="rounded border border-border bg-white px-4 py-2 text-sm font-medium text-text-secondary shadow-sm transition-colors hover:bg-surface">Back to Dashboard</a>
		</div>
	</div>
	...
	<div class="overflow-x-auto rounded-lg border border-border bg-white shadow-sm">
		<table class="min-w-full border-collapse">
			...
			<tbody class="divide-y divide-border-border bg-white">
						...
						<tr class="transition-colors hover:bg-surface">
							...
							<td class="px-6 py-4 text-center whitespace-nowrap">
								...
								{:else if log.actionType === 'OUT'}
									<span class="inline-flex items-center justify-center rounded-full bg-error-light px-2.5 py-0.5 text-xs font-medium text-error-dark uppercase w-20">
										USAGE
									</span>
								{:else}
							...
							<td class="px-6 py-4 text-center text-sm whitespace-nowrap font-bold text-text-secondary">
								{#if log.actionType === 'IN'}
									<span class="text-emerald-600">+{log.quantity}</span>
								{:else if log.actionType === 'OUT'}
									<span class="text-error-dark">-{log.quantity}</span>
								{:else}
									<span>{log.quantity}</span>
								{/if}
								<span class="text-xs text-text-muted font-normal ml-1">{log.unit || ''}</span>
							</td>
				...
				{:else}
					<tr>
						<td colspan="6" class="px-6 py-12 text-center text-sm text-text-muted">
